...
const OrderDAO = require('../models/OrderDAO'); // 1. Import OrderDAO
...
router.put('/customers/:id', JwtUtil.checkToken, async function(req, res) {
  const _id = req.params.id;
  const { username, password, name, phone, email } = req.body;
  const result = await CustomerDAO.update({ _id, username, password, name, phone, email });
  res.json(result);
});

// --------------------------------------------------
// MY CART / CHECKOUT (NEW)
// --------------------------------------------------

router.post('/checkout', JwtUtil.checkToken, async function(req, res) {
  const now = new Date().getTime(); // Lấy thời gian hiện tại (ms)
  const total = req.body.total;
  const items = req.body.items;
  const customer = req.body.customer;

  // Tạo đối tượng đơn hàng với trạng thái mặc định là PENDING
  const order = { 
    cdate: now, 
    total: total, 
    status: 'PENDING', 
